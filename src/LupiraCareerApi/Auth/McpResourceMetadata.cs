...
/// <summary>
/// RFC 9728 protected-resource metadata for the MCP surface (MCP auth spec): a 401 on <c>/mcp</c> points clients
/// here via <c>WWW-Authenticate: … resource_metadata=…</c>, and the document names the Authentik authorization
/// server. Authentik has no dynamic client registration, so clients still pre-register a client id — this only
/// makes discovery, not registration, automatic. Anonymous by design; it discloses nothing but the issuer.
/// </summary>
