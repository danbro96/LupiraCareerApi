...
public class EngagementsTests(CareerApiTestFactory f) : IntegrationTest(f)
{
    ...
    public async Task Create_requires_an_owned_organization()
    {
        ...
        var resp = await api.PostAsJsonAsync("/engagements",
    ...
    public async Task Create_and_get_resolves_organization_name()
    {
        ...
        var got = await api.GetFromJsonAsync<EngagementDto>($"/engagements/{eng.Id}", TestJson.Options);
    ...
    public async Task List_is_newest_first_by_start()
    {
        ...
        var list = await api.GetFromJsonAsync<List<EngagementDto>>("/engagements", TestJson.Options);
    ...
    public async Task Patch_revises_summary_kind_location_and_end()
    {
        ...
        var resp = await api.PatchAsJsonAsync($"/engagements/{eng.Id}",
    ...
    public async Task Titles_assume_then_patch_revise_and_retire()
    {
        ...
        await api.PostAsJsonAsync($"/engagements/{eng.Id}/titles", new AssumeTitleRequest { Text = "Developer", EffectiveFrom = new DateOnly(2020, 1, 1) }, TestJson.Options);
        var afterSecond = await (await api.PostAsJsonAsync($"/engagements/{eng.Id}/titles", new AssumeTitleRequest { Text = "Senior Developer", EffectiveFrom = new DateOnly(2022, 1, 1) }, TestJson.Options)).Content.ReadFromJsonAsync<EngagementDto>(TestJson.Options);
        ...
        var resp = await api.PatchAsJsonAsync($"/engagements/{eng.Id}/titles/{seniorId}", new UpdateTitleRequest { Text = "Staff Engineer", RetiredOn = new DateOnly(2024, 1, 1) }, TestJson.Options);
        resp.EnsureSuccessStatusCode();
    ...
    public async Task Skill_attach_then_detach()
    {
        ...
        var attached = await (await api.PutAsync($"/engagements/{eng.Id}/skills/{skill.Id}", null)).Content.ReadFromJsonAsync<EngagementDto>(TestJson.Options);
        Assert.Contains(skill.Id, attached!.SkillIds);

        var detached = await (await api.DeleteAsync($"/engagements/{eng.Id}/skills/{skill.Id}")).Content.ReadFromJsonAsync<EngagementDto>(TestJson.Options);
    ...
    public async Task Operations_on_a_missing_engagement_are_404()
    {
        ...
        Assert.Equal(HttpStatusCode.NotFound, (await api.GetAsync($"/engagements/{missing}")).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await api.PatchAsJsonAsync($"/engagements/{missing}", new UpdateEngagementRequest { Summary = "x", Kind = null, Location = null, End = null, EndReason = null }, TestJson.Options)).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await api.PostAsJsonAsync($"/engagements/{missing}/titles", new AssumeTitleRequest { Text = "Dev", EffectiveFrom = new DateOnly(2020, 1, 1) }, TestJson.Options)).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await api.PutAsync($"/engagements/{missing}/skills/{Guid.NewGuid()}", null)).StatusCode);
    }
}
